...
import random
from main.EEG.MI_train import BraindecodeTrainer, load_shallowfbcsp_params
...
class EEGSelfDataLoader:
    ...
    def save_as_pt(self):
        self.load_data()
        data_name = rename_file_with_time(config.PT_DATA_FILENAME)
        global_value.train_np_data.append(data_name)  # add the data name in the train np list
        self.x_data, self.y_data = arrange_by_label(self.x_data, self.y_data)
        torch.save({'x_data': self.x_data, 'y_data': self.y_data}, data_name)
        print(f"model save as {data_name}")


# 20251111 新加入讓資料進去是使用各自的 30% 當作 val，而不是全部加起來然後前 30 %
def prepare_datasets_v2(data_x_list, data_y_list, valid_ratio=0.3, segment_len=500, stride=20):  # valid_ratio 介於 0~1
...
class EEGFineTunePipeline:
    def __init__(self, epochs=100, tcp_server=None):  # old: lsl_outlet=None
        self.batch_size = 8
        # self.channel_index = [7, 8, 9, 12, 13, 14, 17, 18, 19, 21, 22, 23, 27, 28, 29]
        self.channel_index = [7, 8, 9, 12, 13, 14, 17, 18, 19, 21, 22, 23, 28]
        self.lr = 1e-3
    ...
    def set_the_ft_set(self):
        ...
        #     fine_tune_datas = [config.FT_CSV_FILENAME]  # config.CSV_FILENAME
        #     fine_tune_logs = [config.FT_LOG_FILENAME]  # config.LOG_FILENAME
        # else:  # 找出 calibration 場景的 Data。Fine tune (csv, log)
        #     fine_tune_datas = [x[0] for x in global_value.data_lookup_table[config.GameSTATE.Calibration.value]]
        #     fine_tune_logs = [x[1] for x in global_value.data_lookup_table[config.GameSTATE.Calibration.value]]
        #
        # print(f"{config.TAGS.INFO.value} fine_tune_datas: {fine_tune_datas}")
        # print(f"{config.TAGS.INFO.value} fine_tune_logs: {fine_tune_logs}")
        # ft_loader = EEGSelfDataLoader(
        #     file_paths=fine_tune_datas,
        #     log_paths=fine_tune_logs,
        #     channel_index=self.channel_index
        #     # [x[0] for x in global_value.data_lookup_table[config.GameSTATE.Calibration.value][0]]
        # )
        # ft_loader.load_data()
        # ft_x, ft_y = ft_loader.get_data()  # (30, 15, 2000) (trial, channel, samples), (30,)
        # ft_x, ft_y = arrange_by_label(ft_x, ft_y)
        data_x_list, data_y_list = [], []
        print(f"{config.TAGS.INFO.value} train_np_data: {global_value.train_np_data}")
        for name in global_value.train_np_data:
            train_data = torch.load(name)
            data_x_list.append(train_data['x_data'])
            data_y_list.append(train_data['y_data'])
        # ft_x = np.concatenate(data_x_list, axis=0)
        # ft_y = np.concatenate(data_y_list, axis=0)
        # print(f"ft_x.shape: {ft_x.shape}")
        # # self.valid_nums 為總資料數量的 3.333 趴，如果除出來不是偶數，那就 +1
        # # 預計會有 20 以上，14 拿來 fine tune，前面 6 組 val 20//3 = 6 val 佔 30 %，下面讀到 data 後會調整
        # valid_nums = ft_x.shape[0] // 3 + 1 if (ft_x.shape[0] // 3) % 2 != 0 else ft_x.shape[0] // 3
        # print(f"valid_nums: {valid_nums}")
        # dataset_ft, dataset_val = prepare_datasets(ft_x, ft_y, valid_num=valid_nums,
        #                                            segment_len=self.segment_len, stride=self.strides)
        print("valid_ratio: 0.3")
        dataset_ft, dataset_val = prepare_datasets_v2(data_x_list, data_y_list, valid_ratio=0.3,
                                                      segment_len=self.segment_len, stride=self.strides)
        return dataset_ft, dataset_val

    def run_calibration(self):
        dataset_ft, dataset_val = self.set_the_ft_set()
        params = load_shallowfbcsp_params(dataset_ft)

        run_braindecode_training(ShallowFBCSPNet, dataset_ft, dataset_val, epochs=self.epochs,
                                 batch_size=self.batch_size, lr=self.lr, freeze_layers=True, params=params,
                                 seed=self.seed, ft=True, tcp_server=self.tcp_server)
...
class EEGTrainingPipeline:
    ...
    def run_training(self):
        ...
        run_braindecode_training(ShallowFBCSPNet, dataset, dataset_val, epochs=self.epochs,
                                 ...
                                 seed=self.seed, ft=False, tcp_server=self.tcp_server)
