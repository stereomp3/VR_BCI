...
# ==========================================
def run_eog_analysis(base_dir=r"/mnt/project/MIEXP/DATA_Cygnus", output_dir="EOG",
                     channels="22", ids=None, sessions=None,
                     log_file="eyes_movement_detect.txt"):
    fs = 500 
    ...
    print("=" * 75)
    print("🚀 Eyes Movement & EOG Cheating Detection 系統啟動")
    print(f"  ├── 資料目錄: {base_dir}")
    print(f"  ├── 輸出目錄: {output_dir}")
    print(f"  ├── 通道模式: {channels} 通道")
    print(f"  └── 處理受試者: {len(ids)} 人 ({', '.join(ids[:5])}{'...' if len(ids)>5 else ''})")
    print("=" * 75)
...
def main():
    parser = argparse.ArgumentParser(description="眼動 EOG 作弊檢定與波形繪製分析系統")
    parser.add_argument("-all", "--all", dest="all_subjects", action="store_true", default=False,
                        help="批次分析全部 24 位受試者")
    ...
    parser.add_argument("--output_dir", type=str, default="EOG",
                        help="EOG 分析圖表輸出目錄 (預設: EOG)")
    parser.add_argument("--channels", type=str, default="22", choices=["13", "22"],
                        help="通道模式 (預設: 22)")
    parser.add_argument("--subject", type=str, default="45",
                        help="單一受試者 ID (預設: 45)")
    parser.add_argument("--ids", type=str, default=None,
                        help="指定分析受試者清單，逗號分隔 (例如: '45,37,70')")
    parser.add_argument("--sessions", type=str, default="s1,s2",
                        help="指定 Session 清單 (預設: s1,s2)")
    parser.add_argument("--log_file", type=str, default="eyes_movement_detect.txt",
                        help="輸出 Log 檔案路徑 (預設: eyes_movement_detect.txt)")
    ...
    if args.ids:
        ...
        target_ids = None  # None loads all 24 subjects
    else:
        target_ids = [args.subject]
