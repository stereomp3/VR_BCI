...
class OnlineCalibrationTrainer(BraindecodeTrainer):
    def __init__(self, dataset=None, val_dataset=None, model_class=ShallowFBCSPNet, model_kwargs=None,
                 batch_size=16, num_epochs=100, lr=1e-4, device=None, ft=False, use_val=True): 
        ...
        if not hasattr(global_value, 'replay_buffer_val'):
    ...
    def add_to_buffer(self, x, y, is_fail):
        ...
        # 1. 將新資料加入 Update Buffer
        global_value.replay_buffer[label_idx].append(new_data)

        # 2. 判斷 Update Buffer 是否超過上限 (FIFO 邏輯)
    ...
    def online_train(self, dataset):
        ...
        # 迴圈結束後，將最佳模型存為 ft-best.pth，並載入回模型
        if best_model_state is not None:
            self.model.load_state_dict(best_model_state)
            torch.save({'model_state_dict': best_model_state}, f"{config.EEG_CHECKPOINT_TMP_BASE_FILE}{tag.lower()}-best.pth")
        print(
