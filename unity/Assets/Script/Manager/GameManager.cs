...
public class GameManager : MonoBehaviour
{
    ...
    public bool is_hand_show_unshow = false; // 開啟這個，雙手靠近手會消失，用在光劍當作手的時候
    ...
    public delegate void SetupAutoSaber(); 
    ...
    void Start()
    {
        ...
        if (is_hand_show_unshow)
        {
            setupAutoSaberCallback += SetupAutoSaberAndHand;
        }
        if (is_audosaber)
        {
            onGameStartCallback += set_hands_unactive;
        }
        LeftHand_parent = LeftHand.transform.parent;
        RightHand_parent = RightHand.transform.parent;
        if (!is_audosaber && AutoSaber.instance != null) AutoSaber.instance.gameObject.SetActive(false);
        SetupAutoSaberAndHand();
    }
    ...
    void SetupAutoSaberAndHand() // 目前在 BeatmapSpawner 觸發 (update)，所以在生成 note 會一直自動把雙手變不見
    {
        ...
        if (AutoSaber.instance != null)
        {
            if(AutoSaber.instance.gameObject.activeSelf == true) return;
            if (is_audosaber) AutoSaber.instance.gameObject.SetActive(true);
            else AutoSaber.instance.gameObject.SetActive(false);
        }
    }
    ...
    public void set_hands_unactive()  // 這個 is_audosaber 加入到 onGameStartCallback，所以 is show hand 其實可以不用
    {
    ...
    {
        is_audosaber = true;
        if (is_audosaber && AutoSaber.instance != null)
        {
            AutoSaber.instance.gameObject.SetActive(true);
            onGameStartCallback += set_hands_unactive;
        }
        SetupAutoSaberAndHand();
